/**
 * LaughTale: Declarative Directives Security & Sandboxing (Hardened Edition)
...
export function sanitizeUrl(url: unknown): string {
    ...
    if (lowerCleaned.startsWith('data:')) {
        ...
        console.warn(`[LaughTale Security] Blocked non-whitelisted data URI: "${trimmed}"`);
        return 'about:blank';
    }
    ...
    try {
        ...
        if (parsed.protocol) {
            ...
            console.warn(`[LaughTale Security] Blocked disallowed protocol "${parsed.protocol}": "${trimmed}"`);
            return 'about:blank';
        }
        ...
        console.warn(`[LaughTale Security] Failed to parse URL: "${trimmed}"`);
        return 'about:blank';
    }
}
    ...
    return new Proxy(state, {
        get(target, prop) {
            if (!isSafeProperty(prop)) {
                console.warn(`[LaughTale Security] Blocked restricted property access: "${String(prop)}"`);
                return undefined;
            }
