...
.dark .laughtale-skeleton {
    background: linear-gradient(90deg, var(--p-surface-100) 25%, var(--p-surface-200) 50%, var(--p-surface-100) 75%) !important;
...
export default function SkeletonIsland(container: HTMLElement, props: SkeletonProps, ctx?: IslandContext) {
    ...
    container.innerHTML = `
        <div class="laughtale-skeleton" data-part="root" style="width: ${width}; height: ${height}; border-radius: ${radius}; background: linear-gradient(90deg, var(--lt-surface-100) 25%, var(--lt-surface-200) 50%, var(--lt-surface-100) 75%); background-size: 200% 100%; animation: skeletonShimmer 1.5s infinite ease-in-out;"></div>
        <style>
