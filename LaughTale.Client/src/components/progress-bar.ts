...
export default function ProgressBarIsland(container: HTMLElement, props: ProgressBarProps, ctx?: IslandContext) {
    ...
    const color = props.color || 'var(--lt-primary-600)';
    ...
    if (isIndeterminate) {
        ...
        container.innerHTML = `
            <div class="laughtale-progress-bar" style="position: relative; height: ${height}; width: 100%; border-radius: 9999px; overflow: hidden; background: var(--lt-surface-100); display: flex; align-items: center;">
                ...
                ${props.showValue !== false && height >= '1rem' ? `
                    <span style="position: absolute; width: 100%; text-align: center; font-size: 0.6875rem; font-weight: 700; color: var(--lt-surface-0, var(--lt-surface-0)); text-shadow: 0 1px 2px rgba(0,0,0,0.5); font-family: var(--p-font-mono);">
                        ${value}%
                    </span>
                ` : ''}
            </div>
