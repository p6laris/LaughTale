...
export default function InputPasswordIsland(container: HTMLElement, props: InputPasswordProps, ctx?: IslandContext) {
    ...
    function render() {
        ...
        const eyeIcon = unsafe(getLucideIcon('eye', 16, 2));
        const eyeOffIcon = unsafe(getLucideIcon('eye-off', 16, 2));
        const clearIcon = unsafe(getLucideIcon('x', 14, 2));

        let leftIconHtml: Raw | '' = '';
        if (props.icon) {
            const iconSvg = getLucideIcon(props.icon);
            leftIconHtml = html`<span class="p-password-left-icon" data-part="root">${iconSvg ? unsafe(iconSvg) : unsafe(getLucideIcon('lock', 16, 2))}</span>`;
        }
                ...
                <div class="p-password-popover" style="display: none;">
                    <div class="p-password-popover-header">
                        <span class="p-password-popover-header-title">
                            ${unsafe(getLucideIcon('shield', 16, 2))}
                            Password Strength
                        </span>
    ...
    function renderRequirementsChips(pwd: string): Raw {
        ...
        const checkIcon = unsafe(getLucideIcon('check', 12, 2.5));
        const xIcon = unsafe(getLucideIcon('x', 12, 2));
    ...
    function renderRequirementsListItems(pwd: string): Raw {
        const r = checkRules(pwd);
        const checkIcon = unsafe(getLucideIcon('check', 14, 2.5));
    ...
    function updateVisuals() {
        ...
        // Update list items
        const listCheckIcon = unsafe(getLucideIcon('check', 14, 2.5));
        const listXIcon = unsafe(getLucideIcon('x', 14, 2));
