...
`;
var CHEVRON_RIGHT_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>`;
function ContextMenuIsland(container, props) {
  ...
  let menuItems = props.model || props.items || [];
  if (menuItems.length === 0) {
    if (demoType === "basic") {
      ...
      menuItems = [
        ...
        { label: "External", icon: "home", url: "https://github.com/SoftMax-Official/SoftMax.LaughTale" }
      ];
    }
  }
  ...
  container.innerHTML = `
    ...
    `;
  const menuEl = container.querySelector("[data-contextmenu-root]");
  ...
  function showMenu(clientX, clientY) {
    ...
    menuEl.querySelectorAll(".p-contextmenu-sublist-wrapper").forEach((sub) => {
  ...
  if (isGlobal) {
    ...
    container.querySelectorAll(".p-contextmenu-product-item").forEach((item) => {
      item.addEventListener("contextmenu", (e) => {
        e.preventDefault();
        e.stopPropagation();
        container.querySelectorAll(".p-contextmenu-product-item").forEach((p) => p.style.borderColor = "transparent");
    ...
    container.querySelectorAll(".p-contextmenu-user-item").forEach((item) => {
      item.addEventListener("contextmenu", (e) => {
        e.preventDefault();
        e.stopPropagation();
        container.querySelectorAll(".p-contextmenu-user-item").forEach((u) => u.style.borderColor = "transparent");
        item.style.borderColor = "var(--p-primary-color, #10b981)";
        selectedTargetEl = item;
        showMenu(e.clientX, e.clientY);
      });
    ...
    const targetBox = container.querySelector("[data-context-target]") || container.querySelector(".p-contextmenu-router-target");
    if (targetBox) {
  ...
  menuEl.querySelectorAll("[data-item-label]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (demoType === "command" && selectedTargetEl) {
        if (label === "Admin" || label === "Member" || label === "Guest") {
          ...
          const toastMsg = window.$toast || window.LaughTaleToast;
          if (toastMsg) {
            toastMsg.add({ severity: "success", summary: "Success", detail: "Invitation sent!", life: 3e3 });
          }
        }
      }
...
//# sourceMappingURL=context-menu-7KBOXHMX.js.map
