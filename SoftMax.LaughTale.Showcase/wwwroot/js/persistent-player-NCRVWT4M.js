...
function PersistentTelemetryIsland(container, props) {
  let requests = 1420;
  let latency = 14;
  container.innerHTML = `
    ...
    `;
  const reqEl = container.querySelector(".req-stat");
  const latEl = container.querySelector(".lat-stat");
  setInterval(() => {
    requests += Math.floor(Math.random() * 9) - 4;
    latency = 12 + Math.floor(Math.random() * 5);
    if (reqEl) reqEl.textContent = `${requests} req/s`;
    if (latEl) latEl.textContent = `${latency}ms`;
  }, (props.refreshIntervalSeconds || 3) * 1e3);
}
export {
  PersistentTelemetryIsland as default
};
//# sourceMappingURL=persistent-player-NCRVWT4M.js.map
