...
// ../SoftMax.LaughTale.Client/src/components/menubar.ts
var MENUBAR_CSS = `
.p-menubar {
    display: flex;
    align-items: center;
...
`;
function MenubarIsland(container, props) {
  ...
  function normalizeItems(rawList) {
    ...
    return rawList.map((it) => ({
      ...
      items: Array.isArray(it.items || it.Items) ? normalizeItems(it.items || it.Items) : void 0
    }));
  }
  const rawData = props.model || props.items || props.Model || props.Items || [];
  const itemsState = normalizeItems(rawData);
  ...
  function renderItem(item, path, isRoot) {
    if (item.separator) {
            ...
            <li class="p-menubar-item ${item.disabled ? "p-disabled" : ""}" role="none" data-path="${path}">
                <div class="p-menubar-item-content">
                    <a class="p-menubar-item-link" role="menuitem" tabindex="-1" href="${item.url || item.route || "#"}" ${item.target ? `target="${item.target}"` : ""}>
                        ...
                        ${item.badge !== void 0 ? `<span class="p-menubar-item-badge">${item.badge}</span>` : ""}
  ...
  function wireEvents(menubarEl2) {
    const button = menubarEl2.querySelector(".p-menubar-button");
    const rootList = menubarEl2.querySelector(".p-menubar-root-list");
    if (button && rootList) {
    ...
    const items = menubarEl2.querySelectorAll(".p-menubar-item");
    items.forEach((li) => {
      const link = li.querySelector(":scope > .p-menubar-item-content > .p-menubar-item-link");
      const submenu = li.querySelector(":scope > .p-menubar-submenu");
      if (!link) return;
      li.addEventListener("mouseenter", () => {
        if (window.innerWidth > 960) {
          const parentUl = li.parentElement;
          if (parentUl) {
            parentUl.querySelectorAll(":scope > .p-menubar-item.p-active").forEach((sibling) => {
    ...
    document.addEventListener("click", (e) => {
      if (!menubarEl2.contains(e.target)) {
        closeAllSubmenus();
    ...
    menubarEl2.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        closeAllSubmenus();
      }
    });
  ...
  const menubarEl = container.querySelector(".p-menubar");
  wireEvents(menubarEl);
}
export {
  MenubarIsland as default
};
//# sourceMappingURL=menubar-ECTNCLLG.js.map
