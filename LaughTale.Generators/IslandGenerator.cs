...
public class IslandGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateTagHelper(IslandModel model)
    {
        ...
        sb.AppendLine("    public override void Process(TagHelperContext context, TagHelperOutput output)");
        sb.AppendLine("    {");
        ...
        sb.AppendLine("        if (httpContext is null || requestServices is null) { output.SuppressOutput(); return; }");
        sb.AppendLine();
        sb.AppendLine("        var evaluator = requestServices.GetService<LaughTale.Core.Security.IIslandAccessEvaluator>();");
        sb.AppendLine("        if (evaluator is null) { output.SuppressOutput(); return; }");
        sb.AppendLine();
        sb.AppendLine($"        var decision = await evaluator.EvaluateAsync(\"{islandDataName}\", httpContext.User, requestServices, new LaughTale.Core.Security.IslandAccessContext(ExplicitPolicy: Policy, LocalOptions: null));");
        sb.AppendLine("        if (!decision.IsAllowed) { output.SuppressOutput(); return; }");
        sb.AppendLine();
