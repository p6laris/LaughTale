    ...
    container.innerHTML = `
        <div class="p-input-group" style="display: flex; flex-direction: column; gap: 1rem;">
            ...
            <div style="display: flex; align-items: center; justify-content: space-between;">
                ...
                <span style="font-size: 0.75rem; color: var(--p-primary-600); font-weight: 500;">Step: ±${step}</span>
            </div>
        </div>
    `;

    const display = container.querySelector('.count-display')!;
    const update = () => { display.textContent = count.toString(); };

    container.querySelector('.increment-btn')?.addEventListener('click', () => { count += step; update(); });
    container.querySelector('.decrement-btn')?.addEventListener('click', () => { count -= step; update(); });
    container.querySelector('.reset-btn')?.addEventListener('click', () => { count = props.initialCount; update(); });
}
