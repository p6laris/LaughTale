...
| **New adapters — [CLOSED except Angular, explicitly deferred - see below] Web Components, Solid, and Alpine all closed** Web Components/Lit first (framework-agnostic, no runtime download), then Solid, Alpine, Angular (unglamorous, but what enterprise .NET shops run). **Web Components** (`adapters/web-components.ts`, new): mounts a real `customElements`/`HTMLElement` instance - genuinely zero-dependency, matching "no runtime download" literally (unlike react/vue/svelte/preact, there is nothing to `import()`), and works identically for a Lit-authored component, since Lit components register via `customElements.define` too - the adapter can't tell the difference and doesn't need to. Reuses the existing `:scope > .island-slot` extraction convention, but simpler than the framework adapters: a real custom element's own Shadow DOM `<slot>` projects light-DOM children natively, so no synthetic slot-host element is needed. Registered in the named adapter registry as `"web-components"` (`getAdapter('web-components')`, ROADMAP.v5.md Part G/L) and exported from both lean entry points (`runtime.ts` AND `runtime-core.ts` - it adds zero weight to the framework-agnostic `runtime.ts` bundle specifically, unlike the other four adapters). Added `IslandFramework.WebComponents` to the C# enum. Live demo: `/polyglot`'s 6th card, a real Shadow-DOM custom element, live-browser verified (renders, increments, zero console errors - `get_page_text` doesn't show its content because that tool doesn't pierce Shadow DOM, confirmed directly via a DOM query instead). 9 new tests.
  **Solid** (`adapters/solid.ts`, new): Solid has no vdom to diff, so React/Vue/Preact's "call render() again with new props" approach doesn't apply - `update()` instead reassigns a `createSignal`-backed Proxy that the mounted component's own tracked reads (an `effect`, or Solid-compiled JSX) subscribe to, patching only the DOM each actually touches, with no new component instance. This is not a workaround: it's the same "props are getters over a reactive source" shape Solid's own compiler generates for `<Widget {...props} />`, written by hand since this adapter has no JSX compiler in its build. A real, non-obvious environment gap found and fixed along the way: solid-js's package.json "exports" map selects a client-only-API-throwing SSR stub under Node's default module resolution condition set (unlike react/vue/preact, which have no such split) - harmless for real consumers (their own bundler resolves "browser" conditions), but broke this repo's own Node-based test runner; fixed by adding `--conditions=browser` to `run-tests.mjs`'s `node --test` invocation, confirmed to not affect any other already-tested dependency. `solid-js` added as a `LaughTale.Client` devDependency. 4 new tests, all passing across 5 different PRNG-unrelated verification runs.
  **Alpine** (`adapters/alpine.ts`, new): architecturally different from every other adapter - Alpine never owns a vdom or re-renders markup, it walks EXISTING DOM for `x-*` directives and binds a reactive data object to them in place (closer to LaughTale's own `l-*` directive engine than to a mount-a-component model), so slot/children forwarding needed by every other adapter simply does not apply here - Alpine never wipes the container's existing markup in the first place. `Component` is instead an "Alpine data factory" (`(props) => object`, the same shape as `Alpine.data(name, factory)`); `update()` mutates the live `Alpine.$data(container)` object in place via `Object.assign`, propagating through Alpine's own reactivity (backed by `@vue/reactivity`, confirmed via its `dependencies`). **A real bug found live, not just in unit tests**: the live Showcase demo threw `ReferenceError: label is not defined` on load - `Alpine.start()` performs its own synchronous initial document walk the first time it runs, and the container's `x-text`/`x-on` markup was already live in the DOM by that point while this adapter's own `x-data`-setting code ran AFTER calling `ensureAlpineStarted`. Fixed by reordering: `Alpine.data(...)` registration and `container.setAttribute('x-data', ...)` now both happen before `ensureAlpineStarted`/`initTree` are ever called, so nothing Alpine-owned ever looks at the container before its scope exists. Verified fixed against the live `/polyglot` page (not just re-reasoned about), both before (reproduced the real error) and after (clean render, real click-driven reactivity through a real `x-on:click` handler) the fix. `alpinejs`/`@types/alpinejs` added as `LaughTale.Client` devDependencies; `happy-dom`'s `ShadowRoot` added to `tests/setup.ts` (Alpine's own `findClosest` needs it unconditionally, not just for Shadow-DOM scenarios). 6 new tests.
  Both new adapters registered in the named adapter registry (`"solid"`, `"alpine"`) and exported from `runtime-core.ts` (not the lean `runtime.ts`, matching react/vue/svelte/preact's own precedent - each pulls in its own framework package via a genuine dynamic `import()`). `IslandFramework.Solid` added to the C# enum (`Alpine` already existed as a declared-but-unimplemented value from an earlier pass - now actually implemented). Live demo: `/polyglot` grew from 6 to 8 cards (Solid Counter, Alpine Counter), both live-browser-verified end to end (render, click, reactive update) via direct DOM queries, not just "the build succeeded." The all-in-one IIFE bundle grew from 373.63 KB to 407.96 KB gzip from inlining both new packages (still under its 420 KB CI budget, but with less headroom - worth knowing before adding an 9th adapter the same way).
  **Angular deliberately NOT attempted** - a real, confirmed toolchain blocker, not a time-boxing choice: modern Angular's `createComponent()` API still requires either AOT-compiled Ivy component definitions (this repo's plain esbuild pipeline has no Angular decorator/template compiler integration - that's normally supplied by the Angular CLI's own `@angular/build` esbuild plugin, a build-tooling addition far beyond an adapter file) or JIT compilation via `@angular/compiler` (esbuild's decorator transpilation does not reproduce Angular's own TypeScript metadata-reflection requirements). Separately, `@angular/core@latest`'s own `peerDependencies` still list `zone.js` - a global monkey-patch of every async browser API on the page the moment it's imported, in real tension with this framework's per-island opt-in architecture (every other adapter's dependency is scoped to only when that specific island's own chunk loads). Angular's own newer zoneless change detection would avoid that second problem, but pins the whole adapter to a recent Angular version and requires the consumer's own component to be authored for it - a real scope decision this roadmap doesn't make. Closer in shape to Part B's handler-level chunk splitting or Part M's state-machine adoption (both explicitly deferred as their own multi-week problem) than to a same-session adapter | ecosystem | S each, Angular XL |
...
## 13. Part H — Security & production — **[CLOSED, all items]**
...
- **Image optimization — [CLOSED (CLS-prevention half), this pass; responsive resize/format conversion explicitly deferred]** *(Astro, Next)*. `ImageDimensionReader` (`LaughTale.Core/Assets/`, new) parses PNG/GIF/JPEG headers directly - no image-processing dependency, reading only the handful of header bytes each format needs, not the whole file. JPEG needed a real marker-walk (SOI → skip APPn/EXIF/other segments → first SOFn), not a fixed offset like PNG/GIF - verified against real System.Drawing-encoded fixture files (not hand-crafted bytes), including one that genuinely carries metadata segments before SOF0. `ImageOptimizationTagHelper` (`lt-optimize` on a plain `<img>`, matching `lt-integrity`'s own attribute-based opt-in shape) auto-stamps real `width`/`height` whenever the author hasn't already set them - the single biggest Cumulative Layout Shift fix an image can get - and defaults `loading="lazy"` unless `lt-priority` is set (matching Next.js's own `<Image priority>` convention; adds `fetchpriority="high"` too). Remote (`http(s)://`) `src` values correctly skip dimension lookup (no local file to read) while still defaulting to lazy loading. Dev-mode warns on a missing `alt`, matching the `data-laughtale-warning-*` convention. Live demo: `/ImageOptimizationDemo`, a real local 800×400 PNG, live-browser verified - `width="800" height="400"` and `loading="lazy"`/`loading="eager"`+`fetchpriority="high"` all present in the actual rendered HTML, none written by hand in the page's own markup. 21 new tests. **Deliberately NOT built**: responsive `srcset` generation and format conversion (WebP/AVIF) - Next.js/Astro's other real "image optimization" half, but each needs an actual image-processing library (a new dependency decision this pass doesn't make) plus a resize/cache pipeline of its own - scoped honestly to the CLS-prevention half, not silently presented as the whole feature.
- **Font optimization — [CLOSED, this pass]** *(Astro, Next)*. Confirmed a real, present gap: both `LaughTale.Showcase` and `LaughTale.Docs` loaded Geist/Inter/JetBrains Mono/Outfit/Plus Jakarta Sans from `fonts.googleapis.com`/`fonts.gstatic.com` on every page load - a third-party network round trip despite the existing preconnect/preload hints, and a visitor IP sent to Google that self-hosting avoids entirely (the same fix `next/font`/Astro's font tooling apply). `scripts/gen-fonts.mjs` (new, repo-root, matching `gen-icons.mjs`'s shared-script precedent) fetches the exact family/weight set already in use, downloads each unique gstatic.com woff2 by URL (content-addressed, so re-running the script against an unchanged font set is a no-op), and rewrites the CSS to reference the self-hosted files. Deliberately an author-run, one-time generator, NOT a step in the normal `npm run build` pipeline - a live network fetch on every build is a real, avoidable CI fragility no other script in this repo accepts; run once, commit the output as real static assets, exactly like the already-committed `Speda-Bold.ttf`. Both `_Layout.cshtml`s now link the self-hosted `fonts.generated.css` instead of the Google CDN; the same `@import` was removed from both `site.css`/`docs.css`. Live-verified: zero requests to `fonts.googleapis.com`/`fonts.gstatic.com` on either app, confirmed via `read_network_requests` after a full page load, all font requests resolving to `localhost` instead.
