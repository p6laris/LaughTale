...
export default function DataViewIsland(container: HTMLElement, props: DataViewProps) {
    ...
    // We assume slot-based templating is done server-side or we just render generic items
    ...
    function render() {
        // Read slot content if provided, but for this generic one we use items
        container.innerHTML = `
    ...
    function bindEvents() {
        container.querySelectorAll('.dataview-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                layout = (e.currentTarget as HTMLElement).dataset.layout as 'grid' | 'list';
                render();
            });
        });
    }
