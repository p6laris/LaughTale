/**
 * SoftMax.LaughTale: Enterprise Hierarchical TreeSelect Component (Aura TreeSelect inspired)
 * Multi-level collapsible tree dropdown with instant search filter, breadcrumb trails, and smooth expand animations.
...
export default function CascadeTreeIsland(container: HTMLElement, props: DepartmentTreeProps) {
        ...
        <div class="laughtale-tree-select" style="position: relative; width: 100%; max-width: 380px; font-family: var(--p-font-family, inherit);">
            ...
            <button type="button" 
                ...
                <div style="display: flex; align-items: center; gap: 0.5rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                    <span style="color: var(--p-primary-600); display: flex;">${LucideIcons.gitBranch || '🌳'}</span>
    ...
    const triggerBtn = container.querySelector<HTMLButtonElement>('.tree-trigger-btn')!;
    const triggerLabel = container.querySelector<HTMLElement>('.tree-trigger-label')!;
    const clearBtn = container.querySelector<HTMLButtonElement>('.tree-clear-btn')!;
    const chevron = container.querySelector<HTMLElement>('.tree-chevron')!;
    const dropdown = container.querySelector<HTMLElement>('.tree-dropdown-menu')!;
    const searchInput = container.querySelector<HTMLInputElement>('.tree-search-input')!;
    const nodesContainer = container.querySelector<HTMLElement>('.tree-nodes-container')!;
    const countLabel = container.querySelector<HTMLElement>('.tree-count-label')!;
    const hiddenInput = container.querySelector<HTMLInputElement>(`#${props.targetInputName || 'tree_selected'}`)!;
    ...
    triggerBtn.addEventListener('click', (e) => {
        if ((e.target as HTMLElement).closest('.tree-clear-btn')) return;
        disclosure.toggle();
    });
    ...
    function renderTree() {
        ...
        function renderNodes(nodes: DepartmentNode[], depth: number, parentEl: HTMLElement) {
            nodes.forEach(node => {
