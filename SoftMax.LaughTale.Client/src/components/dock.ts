/**
 * SoftMax.LaughTale: Enterprise Dock Component (Aura Dock inspired)
 * macOS-style dock menu with dynamic magnification hover effects and tooltips.
 */
...
export default function DockIsland(container: HTMLElement, props: DockProps) {
    ...
    container.innerHTML = `
        <div class="laughtale-dock" style="display: inline-flex; align-items: center; gap: 0.75rem; background: rgba(255, 255, 255, 0.85); dark:bg-slate-900; backdrop-filter: blur(12px); border: 1px solid var(--p-border-color); border-radius: 9999px; padding: 0.5rem 1rem; box-shadow: var(--p-shadow-lg);">
            ${items.map(it => {
                const iconSvg = (LucideIcons as any)[it.icon] || LucideIcons.terminal;
                return `
                    <button type="button" 
                            class="dock-item-btn" 
    ...
    container.querySelectorAll<HTMLButtonElement>('.dock-item-btn').forEach(btn => {
