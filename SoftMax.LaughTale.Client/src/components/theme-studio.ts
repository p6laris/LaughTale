/**
 * SoftMax.LaughTale: TweakAura Live Theme Studio (tweakcn-inspired)
...
export default function ThemeStudioIsland(container: HTMLElement, props: ThemeStudioProps = {}) {
    ...
    container.querySelectorAll('.mode-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            container.querySelectorAll('.mode-btn').forEach(b => {
                ...
                (b as HTMLElement).style.borderColor = 'var(--p-border-color)';
                (b as HTMLElement).style.background = 'var(--p-surface-50)';
            });
            btn.classList.add('active');
            (btn as HTMLElement).style.borderColor = 'var(--p-primary-500)';
            (btn as HTMLElement).style.background = 'var(--p-primary-50)';
            applyTheme();
        });
    });
    ...
    colorGrid.querySelectorAll('.studio-color-swatch').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            colorGrid.querySelectorAll('.studio-color-swatch').forEach(b => {
                ...
                (b as HTMLElement).style.boxShadow = k === currentPrimary ? '0 0 0 2px var(--p-surface-900)' : 'none';
            });
            applyTheme();
        });
    });
    ...
    neutralGrid.querySelectorAll('.studio-neutral-swatch').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            neutralGrid.querySelectorAll('.studio-neutral-swatch').forEach(b => {
                ...
                (b as HTMLElement).style.border = k === currentNeutral ? '2px solid var(--p-primary-500)' : '1px solid var(--p-border-color)';
            });
            applyTheme();
        });
    });
    ...
    container.querySelectorAll('.radius-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            (btn as HTMLElement).style.borderColor = 'var(--p-primary-500)';
            (btn as HTMLElement).style.background = 'var(--p-primary-50)';
            (btn as HTMLElement).style.color = 'var(--p-primary-700)';
