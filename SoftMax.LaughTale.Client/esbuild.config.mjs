...
if (fs.existsSync('dist/runtime.mjs')) {
    ...
    console.log(`[SoftMax.LaughTale] Core Runtime Size: ${rawKb} KB (gzipped: ${gzipKb} KB / budget: ${(RUNTIME_GZIP_BUDGET_BYTES / 1024).toFixed(0)} KB)`);

    if (isProd && gzipBuffer.length > RUNTIME_GZIP_BUDGET_BYTES) {
        console.error(`❌ [SoftMax.LaughTale] Performance Budget Exceeded! Core runtime is ${gzipKb} KB (Budget: ${(RUNTIME_GZIP_BUDGET_BYTES / 1024).toFixed(0)} KB).`);
        process.exit(1);
    } else {
        console.log(`✅ [SoftMax.LaughTale] Performance Budget Passed!`);
    }
}
